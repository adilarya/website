...
const Avatar = () => (
  ...
  >
    ...
    {/* Avatar container */}
    <div className="relative w-[260px] h-[260px] mt-8">
      {/* Glow ring */}
      <div className="absolute inset-0 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 opacity-70 blur-md scale-105" />

      {/* Avatar circle */}
      <div className="relative w-full h-full rounded-full border-2 border-indigo-400/50 overflow-hidden flex items-center justify-center bg-[#12121e]">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/80 via-[#1a1a3e] to-cyan-900/50" />
        <span className="relative z-10 text-7xl font-black text-transparent bg-gradient-to-br from-indigo-300 to-cyan-300 bg-clip-text select-none tracking-tight">
          AA
        </span>
        <div
      ...
      <motion.div
        ...
        className="absolute -top-4 -right-6 px-3 py-1.5 rounded-full text-xs font-semibold border backdrop-blur-sm bg-indigo-500/20 border-indigo-500/40 text-indigo-300"
      >
        GPA 3.903
      </motion.div>

      {/* FIX: was "Columbia '26 →", corrected to "Columbia '28 →" */}
      <motion.div
        animate={{ y: [0, 6, 0] }}
        transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
        className="absolute -bottom-2 -left-8 px-3 py-1.5 rounded-full text-xs font-semibold border backdrop-blur-sm bg-cyan-500/20 border-cyan-500/40 text-cyan-300"
      >
        Columbia '28 →
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-8 -right-10 px-3 py-1.5 rounded-full text-xs font-semibold border backdrop-blur-sm bg-violet-500/20 border-violet-500/40 text-violet-300"
      >
        AI / CV / NLP
      </motion.div>
    </div>
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <motion.div {...fadeUp(0.5)} className="flex flex-wrap items-center gap-3 pt-2">
              <motion.a
                ...
                whileTap={{ scale: 0.97 }}
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white text-sm font-semibold shadow-lg shadow-indigo-500/25 transition-all duration-200"
              >
                Contact Me
              </motion.a>

              {/*
                FIX: was href="#" (broken). Now points to /resume.pdf and opens
                in a new tab. All existing classes/animations are preserved exactly.
              */}
              <motion.a
                href="/resume.pdf"
                target="_blank"
            ...
            <motion.div {...fadeUp(0.6)} className="flex items-center gap-4 pt-1">
              <motion.a
                ...
                className="flex items-center gap-2 text-sm transition-colors text-gray-500 hover:text-gray-200"
              >
                <Github size={16} />
                <span>adilarya</span>
              </motion.a>
              ...
              <a
                ...
                className="text-sm transition-colors text-gray-500 hover:text-gray-200"
              >
