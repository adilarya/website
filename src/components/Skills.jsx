...
const SKILL_GROUPS = [
  {
    ...
    pipActive: 'from-indigo-400 to-cyan-400',
    skills: [
      { name: 'Python',     level: 5 },
      { name: 'C++',        level: 4 },
      { name: 'Java',       level: 3 },
  ...
  },
  {
    ...
    pipActive: 'from-cyan-400 to-teal-400',
    skills: [
  ...
  },
  {
    ...
    pipActive: 'from-violet-400 to-indigo-400',
    skills: [
  ...
  },
  {
    ...
    pipActive: 'from-amber-400 to-orange-400',
    skills: [
...
// ─── Pip ─────────────────────────────────────────────────────────────────────
// Receives the group's `pipActive` gradient so filled pips are color-matched
// to their card. The `groupHovered` prop bumps opacity slightly on hover.
function SkillPip({ filled, pipActive, groupHovered }) {
  return (
    <div
      className={`w-1.5 h-1.5 rounded-full transition-all duration-200 ${
        filled
          ? `bg-gradient-to-r ${pipActive} ${groupHovered ? 'opacity-100' : 'opacity-80'}`
          : `bg-white/10 ${groupHovered ? 'bg-white/15' : ''}`
      }`}
    />
  )
}
...
// ─── Skill row ───────────────────────────────────────────────────────────────
// Previously completely static. Now:
//  - `group` on the wrapper div → CSS hover target
//  - Name text lifts from gray-300 → white on hover
//  - Pips get a subtle brightness bump (handled via groupHovered state is
//    unnecessary at scale; we use CSS group-hover instead for performance)
function SkillRow({ skill, pipActive, delay, isInView }) {
  return (
    <motion.div
      initial={{ opacity: 0, x: -10 }}
      animate={isInView ? { opacity: 1, x: 0 } : {}}
      transition={{ duration: 0.4, delay }}
      // group — drives the CSS transitions on name + pips below
      className="group flex items-center justify-between py-0.5 rounded-md px-1 -mx-1 cursor-default
      ...
      <div className="flex items-center gap-1">
        {[1, 2, 3, 4, 5].map(pip => (
          <div
            ...
            className={`w-1.5 h-1.5 rounded-full transition-all duration-200 ${
              pip <= skill.level
                ? `bg-gradient-to-r ${pipActive} group-hover:opacity-100 opacity-75`
